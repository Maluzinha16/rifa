<?php
    ...
    if(empty($requestData['TITULO'])){
        ...
        if($operacao == 'insert'){
            ...
            try{
                $stmt = $pdo->prepare('INSERT INTO PROMOCAO (TITULO, DESCRICAO, DATA_INICIO, DATA_FIM,DATA_SORTEIO,  VALOR_RIFA ) VALUES (:a, :b, :c, :d, :e,  :g)');
                $stmt->execute(array(
            ...
            try{
                $stmt = $pdo->prepare('UPDATE PROMOCAO SET TITULO = :a, DESCRICAO = :b, DATA_INICIO = :c, DATA_FIM = :d, DATA_SORTEIO = :e, VALOR_RIFA = :g  WHERE ID = :id');
                $stmt->execute(array(
                    ':id' => $ID,
                     //':a' => utf8_decode($requestData['TITULO'])
                     ':a' => $requestData['TITULO'],
